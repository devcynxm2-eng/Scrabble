...
//     [Tooltip(
//         "Cube ki jagah rocket ka actual visual prefab. Is prefab ko runtime " +
//         "rocket root ke neeche instantiate kiya jayega."
//     )]
//     [SerializeField]
//     private GameObject rocketVisualPrefab;

//     [Tooltip(
//         "Imported rocket mesh ko flight ke forward (+Z) axis ke sath align karta hai."
//     )]
//     [SerializeField]
//     private Vector3 rocketVisualLocalEulerAngles =
//         new Vector3(90f, 0f, 0f);

//     [SerializeField]
//     private Vector3 rocketVisualLocalScale =
//         Vector3.one * 0.25f;
...
//             rocketVisualPrefab,
//             rocketVisualLocalEulerAngles,
//             rocketVisualLocalScale
//         );
//     }
...
//         GameObject requestedRocketVisualPrefab,
//         Vector3 requestedRocketVisualLocalEulerAngles,
//         Vector3 requestedRocketVisualLocalScale)
//     {
//         rocketVisualPrefab = requestedRocketVisualPrefab;
//         rocketVisualLocalEulerAngles =
//             requestedRocketVisualLocalEulerAngles;
//         rocketVisualLocalScale =
//             requestedRocketVisualLocalScale;
...
//             visual.name = "Rocket Visual";
//             visual.transform.localPosition = Vector3.zero;
//             visual.transform.localRotation = Quaternion.Euler(
//                 rocketVisualLocalEulerAngles
//             );
//             visual.transform.localScale = rocketVisualLocalScale;
...
//         else
//         {
//             /* Missing prefab ke liye safe legacy fallback. */
//             CreatePrimitivePart(
//                 PrimitiveType.Cube,
//                 rocket.transform,
//                 "Fallback Rocket Cube",
//                 Vector3.zero,
//                 Vector3.one * 0.32f,
//                 Quaternion.identity,
//                 rocketMaterial
...
//     [Tooltip("Cannon ke left/right launcher placement ka anchor / recoil root.")]
//     [SerializeField] private Transform cannonVisualRoot;


//     [Tooltip(
//         "Rocket ka actual launch point. Cannon barrel ke bilkul end par " +
//         "Empty GameObject rakhein aur yahan assign karein. Agar empty ho " +
//         "to CannonController.RocketLaunchPoint fallback use hoga."
//     )]
//     [SerializeField] private Transform rocketMuzzlePoint;
...
//         Transform rocketLaunchPoint =
//             GetRocketLaunchPoint();

//         if (rocketBarrage != null)
//         {
//             if (rocketLaunchPoint == null)
//             {
//                 Debug.LogWarning(
//                     "GameplayPowerUpController: Rocket Muzzle Point " +
//                     "assign nahi hai aur CannonController.RocketLaunchPoint " +
//                     "bhi available nahi hai.",
//                     this
//                 );
//             }
//             else
...
//             powerCannonAwaitingShot = false;
//             Debug.LogWarning(
//                 "GameplayPowerUpController: Rocket launch point missing hai.",
//                 this
//             );
//             return;
//         }
...
//     /// <summary>
//     /// Rocket ka actual launch point resolve karta hai.
//     /// Inspector mein Rocket Muzzle Point assign ho to wahi use hoga.
//     /// Agar assign nahi hai to existing CannonController point fallback hai.
//     /// </summary>
//     private Transform GetRocketLaunchPoint()
//     {
//         if (rocketMuzzlePoint != null)
//         {
//             return rocketMuzzlePoint;
//         }

//         if (cannonController != null)
//         {
//             return cannonController.RocketLaunchPoint;
//         }
...
//   Vector3 rollReferenceUp = Vector3.up;
...
//              */
//             rocket = existingRocket;
//  rollReferenceUp = rocket.transform.up;
//             rocket.transform.SetParent(
//                 parent,
//                 true
//             );
...
//                 /*
//                  * FIX: Quaternion.LookRotation degenerate/glitch ho jata
//                  * hai jab forward direction world-up ke bilkul parallel
//                  * ho (matlab steep/near-vertical shot). Us case mein
//                  * up-reference ko Vector3.forward par switch karte hain
//                  * taake rotation kabhi bhi undefined/tedhi na ho.
//                  */
//                 Vector3 upReference =
//                     Mathf.Abs(
//                         Vector3.Dot(
...
public sealed class GameplayPowerUpController : MonoBehaviour
{
    ...
    [Tooltip(
        "Rocket impact par play hone wala ParticleSystem prefab. Empty ho " +
        "to built-in runtime explosion use hogi."
    )]
    [SerializeField]
    private ParticleSystem rocketExplosionParticlePrefab;
...
internal sealed class RuntimeRocketBarrage : MonoBehaviour
{
    ...
    public void Configure(
        ...
        float requestedArmedForwardOffset,
        GameObject requestedRocketVisualPrefab,
        ParticleSystem requestedRocketExplosionParticlePrefab,
        Vector3 requestedRocketVisualLocalEulerAngles,
        Vector3 requestedRocketVisualLocalScale)
    {
        rocketVisualPrefab = requestedRocketVisualPrefab;
        rocketExplosionParticlePrefab =
            requestedRocketExplosionParticlePrefab;
        rocketVisualLocalEulerAngles =
            requestedRocketVisualLocalEulerAngles;
        rocketVisualLocalScale =
            requestedRocketVisualLocalScale;
        ...
        GameObject existingRocket)
    {
        ...
        /*
         * FIX: armed state (muzzle par loaded) mein rocket ka roll/up
         * jo bhi tha, wahi yahan capture karte hain — REPARENT karne se
         * pehle. Isse flight ke pehle frame mein rotation "snap" nahi
         * hogi, balki wahin se continue hogi jahan armed state chhoti thi.
         */
        Quaternion lockedFlightRotation;
        ...
        if (existingRocket != null)
        {
            ...
            lockedFlightRotation = rocket.transform.rotation;
            ...
            rocket.transform.position = startPosition;
            rocket.transform.rotation = lockedFlightRotation;
        }
        else
        {
            rocket = CreateRocket(
            ...
            Vector3 initialDirection =
                impactPoint - startPosition;

            lockedFlightRotation =
               ...
               rocket != null)
        {
            ...
            // Straight flight mein launch wali orientation lock rehti hai.
            rocket.transform.rotation = lockedFlightRotation;
            elapsed += Time.deltaTime;
            yield return null;
        }
    ...
    private void CreateExplosion(Vector3 position)
    {
        if (rocketExplosionParticlePrefab != null)
        {
            ParticleSystem customParticles = Instantiate(
                rocketExplosionParticlePrefab,
                position,
                Quaternion.identity
            );

            customParticles.gameObject.SetActive(true);
            customParticles.Play(true);

            ParticleSystem.MainModule customMain =
                customParticles.main;

            float destroyDelay = Mathf.Max(
                0.25f,
                customMain.duration +
                customMain.startLifetime.constantMax +
                0.25f
            );

            Destroy(customParticles.gameObject, destroyDelay);
            return;
        }
