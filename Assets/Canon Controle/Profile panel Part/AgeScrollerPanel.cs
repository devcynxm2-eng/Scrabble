...
///
/// SETUP:
///   ScrollRect + Content, an UpButton/DownButton (kept as optional/hidden),
///   a SelectedAgeLabel, and a ConfirmButton. Items are generated at runtime in BuildList().
///
/// INTERACTION:
///   User drags/swipes the ScrollRect directly. On release, it snaps to the
///   nearest age (one item at a time), same as the old arrow-button behaviour.
/// </summary>
public class AgeScrollerPanel : OnboardingPanelBase
{
    [Header("References")]
    ...
    [Header("Arrow Buttons (hidden, kept for optional re-enable)")]
    [SerializeField] private Button upButton;
    [SerializeField] private Button downButton;
    [SerializeField] private bool showArrowButtons = false;
    ...
    private DragEndRelay dragRelay;
    ...
    protected override void OnPanelShown()
    {
        ...
        if (scrollRect != null)
        {
            // Allow the user to drag/swipe the list directly.
            scrollRect.vertical = true;
            scrollRect.horizontal = false;
            scrollRect.inertia = false; // prevent momentum from skipping past several ages
            scrollRect.movementType = ScrollRect.MovementType.Clamped;

            scrollRect.onValueChanged.RemoveListener(OnScrollValueChanged);
            scrollRect.onValueChanged.AddListener(OnScrollValueChanged);

            dragRelay = scrollRect.GetComponent<DragEndRelay>();
            if (dragRelay == null)
                dragRelay = scrollRect.gameObject.AddComponent<DragEndRelay>();

            dragRelay.onBeginDrag = OnUserBeginDrag;
            dragRelay.onEndDrag = OnUserEndDrag;
        }
        ...
        // Arrow buttons are hidden — scrolling/dragging is the only interaction now.
        if (upButton != null)
        {
            upButton.onClick.RemoveAllListeners();
            upButton.onClick.AddListener(OnTopButtonClicked);
            upButton.gameObject.SetActive(showArrowButtons);
        }
        ...
        if (downButton != null)
        {
            downButton.onClick.RemoveAllListeners();
            downButton.onClick.AddListener(OnBottomButtonClicked);
            downButton.gameObject.SetActive(showArrowButtons);
        }
    ...
    // ---------- Drag handling (replaces button-driven movement) ----------

    private void OnUserBeginDrag()
    {
        if (isSnapping)
            content.DOKill();
    ...
    private void OnUserEndDrag()
    {
        isDragging = false;

        int nearest = GetNearestIndexFromPosition();
        SnapToIndex(nearest);
    }

    private void OnScrollValueChanged(Vector2 normalizedPos)
    {
        // While the user is manually dragging (or ScrollRect is settling),
        // keep the wheel visuals (scale/alpha/color) in sync.
        if (!isSnapping)
            RefreshVisuals();
    }

    private int GetNearestIndexFromPosition()
    {
        RectTransform viewport = scrollRect != null ? scrollRect.viewport : null;
        ...
        return Mathf.Clamp(Mathf.RoundToInt(centerIndex), 0, items.Count - 1);
    }

    // ---------- Optional arrow-button support (currently hidden) ----------

    private void OnTopButtonClicked()
    {
        if (isSnapping || isDragging || items.Count == 0)
            return;
    ...
    /// <summary>
    /// Small relay so we can hear the ScrollRect's begin/end drag events
    /// without having AgeScrollerPanel itself sit on the same GameObject
    /// as the ScrollRect.
    /// </summary>
    private class DragEndRelay : MonoBehaviour, IBeginDragHandler, IEndDragHandler
    {
