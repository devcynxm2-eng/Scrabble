...
//     [SerializeField, Min(0f)] private float depthGap = 0.04f;
...
//         DrawPropertyField("depthGap", "Layer Distance");
...
//         level.EditorSetDepthGap(depthGap);
...
[SerializeField, Min(1)] private int deleteRangeStart = 1;
[SerializeField, Min(1)] private int deleteRangeEnd = 1000;
    ...
    [Tooltip(
        "Delete Generated Level section ke liye - kaunsa level number " +
        "delete karna hai."
    )]
    [SerializeField, Min(1)] private int deleteLevelNumber = 1;
    ...
    private void OnGUI()
    {
        ...
        EditorGUILayout.Space(10f);
        EditorGUILayout.LabelField("Delete Generated Level", EditorStyles.boldLabel);
        ...
        EditorGUILayout.HelpBox(
            "Deletes one already-generated level: removes it from the " +
            "Addressables group, deletes its .asset file from disk, " +
            "and removes its entry from the level database. Not " +
            "undoable.",
            MessageType.Warning
        );

        DrawPropertyField("deleteLevelNumber", "Level Number");

        using (new EditorGUI.DisabledScope(deleteLevelNumber < 1))
        {
            if (GUILayout.Button("DELETE LEVEL", GUILayout.Height(32f)))
            {
                DeleteGeneratedLevel(deleteLevelNumber);
            }
        }
...
EditorGUILayout.LabelField("Delete Range", EditorStyles.miniLabel);
DrawPropertyField("deleteRangeStart", "From Level");
DrawPropertyField("deleteRangeEnd", "To Level");

if (GUILayout.Button("DELETE RANGE", GUILayout.Height(28f)))
{
    DeleteLevelRange(deleteRangeStart, deleteRangeEnd);
}
...
if (GUILayout.Button("DELETE ALL LEVELS", GUILayout.Height(32f)))
{
    DeleteAllLevels();
}
...
GUI.backgroundColor = Color.white;
    ...
    /// <summary>
    /// Deletes one already-generated level: removes its entry from the
    /// Addressables group, deletes the .asset file from disk, and
    /// removes its catalog entry from GridLevelDatabase. Uses the same
    /// database path GetOrCreateLevelDatabase() resolves to, so it
    /// always targets the actual project database rather than creating
    /// a fresh empty one (which would be wrong in a delete context).
    /// </summary>
    private void DeleteGeneratedLevel(int levelNumber)
    {
        GridLevelDatabase database =
            AssetDatabase.LoadAssetAtPath<GridLevelDatabase>(
                GridLevelAddressablesEditorUtility.DatabasePath
        ...
        if (database == null)
        {
            EditorUtility.DisplayDialog(
                "Cannot Delete",
                "Level database asset nahi mila.",
                "OK"
            );
        ...
        if (existingIndex < 0)
        {
            EditorUtility.DisplayDialog(
                "Cannot Delete",
                $"Level {levelNumber} database mein nahi mila.",
                "OK"
            );
        ...
        if (!EditorUtility.DisplayDialog(
                "Delete Level",
                $"Level {levelNumber} permanently delete ho jayega " +
                "(Addressables entry + .asset file + database entry). " +
        ...
        bool success =
            database.EditorDeleteLevel(levelNumber);

        if (success)
        {
            Debug.Log($"Level {levelNumber} delete ho gaya.");
        }
...
private void DeleteLevelRange(int rangeStart, int rangeEnd)
{
    if (rangeStart < 1 || rangeEnd < rangeStart)
    {
        EditorUtility.DisplayDialog(
            "Cannot Delete",
            "From Level must be >= 1 and To Level must be >= From Level.",
            "OK"
        );
    ...
    if (!EditorUtility.DisplayDialog(
            "Delete Level Range",
            $"Levels {rangeStart}-{rangeEnd} permanently delete ho " +
            "jayenge (jitne is range mein maujood hain). Ye undo " +
            "nahi ho sakta.",
            "Delete",
            "Cancel"))
    {
    ...
    int deletedCount =
        database.EditorDeleteLevelRange(rangeStart, rangeEnd);

    Debug.Log($"{deletedCount} level(s) delete ho gaye (range {rangeStart}-{rangeEnd}).");
...
private void DeleteAllLevels()
{
    ...
    if (database.Count == 0)
    {
        EditorUtility.DisplayDialog(
            "Nothing To Delete",
            "Database mein koi level nahi hai.",
            "OK"
        );
    ...
    /*
     * Extra-strong confirmation since this wipes the entire catalog -
     * a single click on the normal dialog would be too easy to hit
     * by accident.
     */
    if (!EditorUtility.DisplayDialog(
            "Delete ALL Levels",
            $"Database ke SAARE {database.Count} level(s) permanently " +
            "delete ho jayenge (Addressables entries + .asset files + " +
            "database entries). Ye undo NAHI ho sakta.",
    ...
    if (!EditorUtility.DisplayDialog(
            "Are You Sure?",
            "Ek aakhri confirmation - poori level database khaali ho " +
            "jayegi. Continue karein?",
            "Delete All",
            "Cancel"))
    {
        return;
    ...
    int deletedCount =
        database.EditorDeleteAllLevels();

    Debug.Log($"{deletedCount} level(s) delete ho gaye (all levels).");
